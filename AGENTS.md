...
允许管理员导入历史提交记录，用于补全成员贡献统计数据。

**功能特性**:

- 选择时间范围，自动从 Git 平台拉取该时间段内的所有提交
- 支持 GitLab、GitHub、Bitbucket 三个平台
- 自动跳过已存在的提交（根据 commit_hash 去重）
- 导入的记录标记为 `is_manual=true`
...
1. 在「项目管理」页面，点击项目操作栏的上传图标
2. 选择开始日期和结束日期
3. 点击确定，系统自动从 Git 平台拉取提交

**前置条件**:

- 项目必须配置 Access Token（用于调用 Git 平台 API）
- 只有管理员可以执行此操作

**API**:

- `POST /api/review-logs/import` - 导入提交（Admin only）

**涉及文件**:

- `/backend/internal/services/import_commits.go` - 导入服务
- `/backend/internal/handlers/review_log.go` - `ImportCommits` handler
- `/frontend/src/pages/Projects.tsx` - 补录提交弹窗
